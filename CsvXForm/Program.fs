...
module CsvXForm =
    ...
    let readVerticalCsv (filePath: string) : Order list =
        ...
        // Initialize orders
        let orders = Array.init orderCount (fun _ -> { 
            OrderID = ""; OrderDateTime = DateTime.MinValue; 
            Customer = { Id = ""; Name = ""; Address = ""; State = ""; City = ""; Zip = "" };
            DeliveryAddress = { Name = ""; Address = ""; State = ""; City = ""; Zip = "" };
            Items = []; TotalDue = 0M; InitialDue = 0M; Terms = ""; Notes = "" })
            ...
            for i in 0 .. orderCount - 1 do
                if i + 1 < rowValues.Length then
                    ...
                    if not (String.IsNullOrEmpty value) then
                        ...
                        | "OrderDateTime" -> orders.[i] <- { order with OrderDateTime = DateTime.Parse(value) }
                        | "Customer.Id" -> orders.[i] <- { order with Customer = { order.Customer with Id = value } }
                        | "Customer.Name" -> orders.[i] <- { order with Customer = { order.Customer with Name = value } }
                        | "Customer.Address" -> orders.[i] <- { order with Customer = { order.Customer with Address = value } }
                        | "Customer.State" -> orders.[i] <- { order with Customer = { order.Customer with State = value } }
                        | "Customer.City" -> orders.[i] <- { order with Customer = { order.Customer with City = value } }
                        | "Customer.Zip" -> orders.[i] <- { order with Customer = { order.Customer with Zip = value } }
                        | "DeliveryAddress.Name" -> orders.[i] <- { order with DeliveryAddress = { order.DeliveryAddress with Name = value } }
                        | "DeliveryAddress.Address" -> orders.[i] <- { order with DeliveryAddress = { order.DeliveryAddress with Address = value } }
                        | "DeliveryAddress.State" -> orders.[i] <- { order with DeliveryAddress = { order.DeliveryAddress with State = value } }
                        | "DeliveryAddress.City" -> orders.[i] <- { order with DeliveryAddress = { order.DeliveryAddress with City = value } }
                        | "DeliveryAddress.Zip" -> orders.[i] <- { order with DeliveryAddress = { order.DeliveryAddress with Zip = value } }
                        | "TotalDue" -> orders.[i] <- { order with TotalDue = decimal value }
                        | "InitialDue" -> orders.[i] <- { order with InitialDue = decimal value }
                        ...
                        | field when Regex.IsMatch(field, @"Item\[\d+\]\.") ->
                            ...
                            while orders.[i].Items.Length <= itemIndex do
                                orders.[i] <- { order with Items = orders.[i].Items @ [{ SKU = ""; Quantity = 0; Price = 0M; LineSubTotal = 0M; LineTaxes = []; LineTaxTotal = 0M; LineTotal = 0M }] }

                            let item = orders.[i].Items.[itemIndex]
                            match itemField with
                            | "SKU" -> 
                                orders.[i] <- { order with Items = orders.[i].Items |> List.mapi (fun j it -> if j = itemIndex then { it with SKU = value } else it) }
                            | "Quantity" -> 
                                orders.[i] <- { order with Items = orders.[i].Items |> List.mapi (fun j it -> if j = itemIndex then { it with Quantity = int value } else it) }
                            | "Price" -> 
                                orders.[i] <- { order with Items = orders.[i].Items |> List.mapi (fun j it -> if j = itemIndex then { it with Price = decimal value } else it) }
                            | "LineSubTotal" -> 
                                orders.[i] <- { order with Items = orders.[i].Items |> List.mapi (fun j it -> if j = itemIndex then { it with LineSubTotal = decimal value } else it) }
                            | "LineTaxTotal" -> 
                                orders.[i] <- { order with Items = orders.[i].Items |> List.mapi (fun j it -> if j = itemIndex then { it with LineTaxTotal = decimal value } else it) }
                            | "LineTotal" -> 
                                orders.[i] <- { order with Items = orders.[i].Items |> List.mapi (fun j it -> if j = itemIndex then { it with LineTotal = decimal value } else it) }
                            | field when Regex.IsMatch(field, @"LineTax\[\d+\]\.") ->
                                ...
                                let updatedItem = 
                                    if item.LineTaxes.Length <= taxIndex then
                                        { item with LineTaxes = item.LineTaxes @ List.replicate (taxIndex - item.LineTaxes.Length + 1) { Jurisdiction = ""; Amount = 0M } }
                                    else item

                                let updatedTax = 
                                    match taxField with
                                    | "Jurisdiction" -> { updatedItem.LineTaxes.[taxIndex] with Jurisdiction = value }
                                    | "Amount" -> { updatedItem.LineTaxes.[taxIndex] with Amount = decimal value }
                                    | _ -> updatedItem.LineTaxes.[taxIndex]

                                orders.[i] <- { order with Items = orders.[i].Items 
                                                                    |> List.mapi (fun j it -> 
                                                                            if j = itemIndex then 
                                                                                { updatedItem with LineTaxes = updatedItem.LineTaxes |> List.mapi (fun k t -> if k = taxIndex then updatedTax else t) } 
                                                                            else it) 
                                                                }
                            | _ -> ()
                        | _ -> ()
